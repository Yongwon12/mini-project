...
const getUser = async () => {
  ...
  // 목록 화면을 완성해주세요.
  let users = await axios.get("http://localhost:5500/user");
  users = users.data;
  const usersCopy = JSON.stringify(users);
  const usersDeepCopy = JSON.parse(usersCopy);
  const usersObj = {};
  for (let i = 0; i < usersDeepCopy.length; i++) {
    usersObj.name = usersDeepCopy[i].name;
    console.log(usersDeepCopy[i].name);
    usersObj.email = usersDeepCopy[i].email;
    usersObj.personal = usersDeepCopy[i].personal;
    usersObj.phone = usersDeepCopy[i].phone;
    usersObj.prefer = usersDeepCopy[i].prefer;
    usersObj.og = usersDeepCopy[i].og;
  }
  createUserDiv(usersObj);
};
